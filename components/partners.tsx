export function Partners() {
  ...
  return (
    <section className="py-16 bg-gradient-to-b from-blue-900 via-blue-800 to-blue-900 overflow-hidden">
      <div className="container mx-auto px-4">
        <h2 className="text-3xl font-bold text-center mb-12 text-white">We Associated With</h2>
        <div className="relative w-full overflow-hidden">
          <div className="flex w-max animate-scroll">
            {[...partners, ...partners].map((partner, index) => (
              <div 
                key={index} 
                className="p-4 bg-white rounded-xl shadow-sm hover:shadow-lg transition-shadow transform hover:scale-105 duration-300 flex items-center justify-center mx-4 min-w-[200px]">
                <img src={partner.logo} alt={partner.name} className="h-12 w-25" />
              </div>
            ))}
          </div>
        </div>
