...
export function ProcessSteps() {
  const steps = [
    ...
    {
      icon: <Check className="w-12 h-12 text-black" />,
      title: "Step 3",
  ...
  return (
    <section className="py-16 bg-gradient-to-b from-blue-900 via-blue-800 to-blue-900 perspective-1000">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {steps.map((step, index) => (
            <div key={index} className="text-center flex flex-col items-center">
              ...
              <p className="text-white text-center max-w-xs">{step.description}</p>
            </div>
          ))}
        </div>
      </div>
